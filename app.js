...
const STARRED = [
  { ticker: "T1", name: "T1 — League of Legends", sector: "LoL", price: 1845, mu: 0.06, sigma: 0.18 },
  { ticker: "GENG", name: "Gen.G — League of Legends", sector: "LoL", price: 1810, mu: 0.05, sigma: 0.16 },
  { ticker: "G2", name: "G2 Esports — League of Legends", sector: "LoL", price: 1760, mu: 0.04, sigma: 0.2 },
  { ticker: "FAZE", name: "FaZe Clan — Counter-Strike 2", sector: "CS2", price: 1820, mu: 0.03, sigma: 0.17 },
  { ticker: "NAVI", name: "NAVI — Counter-Strike 2", sector: "CS2", price: 1855, mu: 0.05, sigma: 0.15 },
  { ticker: "VITA", name: "Team Vitality — Counter-Strike 2", sector: "CS2", price: 1790, mu: 0.04, sigma: 0.19 },
  { ticker: "LOUD", name: "LOUD — Valorant", sector: "Valorant", price: 1745, mu: 0.07, sigma: 0.22 },
  { ticker: "SEN", name: "Sentinels — Valorant", sector: "Valorant", price: 1690, mu: 0.02, sigma: 0.24 },
  { ticker: "FNC", name: "Fnatic — Valorant", sector: "Valorant", price: 1715, mu: 0.05, sigma: 0.21 },
  { ticker: "TS", name: "Team Spirit — Dota 2", sector: "Dota 2", price: 1900, mu: 0.08, sigma: 0.2 },
];
  ...
  "T1": [
    {
      ...
      "author": "@lck_analyst",
      "handle": "1h",
      "text": "$T1 Faker still topside MVP candidate. Worlds odds shortening."
    },
    {
      "sent": "pos",
      "author": "@esp_capital",
      "handle": "5h",
      "text": "$T1 has best macro in LCK by far. Late-game synergy unmatched."
    },
    {
      "sent": "neu",
      "author": "@solo_q",
      "handle": "1d",
      "text": "$T1 jungle pathing inconsistent in regular season but locks in for finals."
    }
  ],
  "GENG": [
    {
      ...
      "author": "@lck_caster",
      "handle": "3h",
      "text": "$GENG laning phase the cleanest in LCK. Win condition almost guaranteed."
    },
    {
      "sent": "neg",
      "author": "@meta_watcher",
      "handle": "11h",
      "text": "$GENG meta-dependent. New patch could swing them down."
    }
  ],
  "G2": [
    {
      ...
      "author": "@lec_diehard",
      "handle": "2h",
      "text": "$G2 looking like the LEC team that can finally translate to Worlds."
    },
    {
      "sent": "neu",
      "author": "@reg_analyst",
      "handle": "9h",
      "text": "$G2 form variable. Top-half-of-LEC strong but Bo5 still wobbly."
    }
  ],
  "FAZE": [
    {
      ...
      "author": "@cs_major",
      "handle": "4h",
      "text": "$FAZE roster shuffle paying off. T-side discipline back to S-tier."
    },
    {
      "sent": "neu",
      "author": "@hltv_writer",
      "handle": "12h",
      "text": "$FAZE on uptrend but vs NAVI/Vitality in playoffs is a coinflip."
    }
  ],
  "NAVI": [
    {
      ...
      "author": "@cs2_lab",
      "handle": "1h",
      "text": "$NAVI s1mple back to peak form. Aim duels north of 60% on Mirage."
    },
    {
      "sent": "pos",
      "author": "@gaming_lp",
      "handle": "10h",
      "text": "$NAVI Russian-speaking roster cohesion remains an edge."
    }
  ],
  "VITA": [
    {
      ...
      "author": "@vita_fan",
      "handle": "5h",
      "text": "$VITA ZywOo carrying. Supporting roles inconsistent."
    }
  ],
  "LOUD": [
    {
      ...
      "author": "@val_brazil",
      "handle": "2h",
      "text": "$LOUD aspas still the best duelist in the world. Tier 1 lock for Champions."
    },
    {
      "sent": "pos",
      "author": "@vct_data",
      "handle": "7h",
      "text": "$LOUD post-plant setups have evolved. Most adaptive team in the league."
    }
  ],
  "SEN": [
    {
      ...
      "author": "@val_drama",
      "handle": "3h",
      "text": "$SEN roster instability again. TenZ minutes inconsistent."
    },
    {
      "sent": "neu",
      "author": "@vct_writer",
      "handle": "1d",
      "text": "$SEN highs are top 3, lows are middle of Americas. Wide variance."
    }
  ],
  "FNC": [
    {
      ...
      "author": "@emea_val",
      "handle": "4h",
      "text": "$FNC Boaster IGL still elite. Map pool deeper than any EMEA team."
    }
  ],
  "TS": [
    {
      ...
      "author": "@dota_invest",
      "handle": "1h",
      "text": "$TS Team Spirit drafting flexibility is the highest in the world. Yatoro form back."
    },
    {
      "sent": "pos",
      "author": "@TI_picker",
      "handle": "8h",
      "text": "$TS odds at the next major shortening daily. Back at +250 was a gift."
    }
  ]
};
...
const STRATEGIES = [
  {
    ...
    apply: (ctx) => {
      ...
      if (s20 == null || s50 == null) return { signal: "EVEN", detail: "Not enough history for 50-period MA." };
      if (s20 > s50 * 1.005) return { signal: "FAVOURED",  detail: `20-period (${s20.toFixed(2)}) above 50-period (${s50.toFixed(2)}) — rising trend.` };
      if (s20 < s50 * 0.995) return { signal: "UNDERDOG", detail: `20-period (${s20.toFixed(2)}) below 50-period (${s50.toFixed(2)}) — falling trend.` };
      return { signal: "EVEN", detail: `20-period ≈ 50-period — consolidation.` };
    },
  },
  {
    id: "rsi_mr",
  ...
  },
  {
    ...
    apply: (ctx) => {
      ...
      if (ratio > 1.3 && expectedReturn > 0.01) return { signal: "FAVOURED", detail: `Up/down ratio ${ratio.toFixed(2)}x, Mean ${(expectedReturn * 100).toFixed(1)}%, Pr(up) ${(probUp * 100).toFixed(0)}%.` };
      if (ratio < 0.8 && expectedReturn < -0.01) return { signal: "UNDERDOG", detail: `Up/down ratio ${ratio.toFixed(2)}x, Mean ${(expectedReturn * 100).toFixed(1)}%, Pr(up) ${(probUp * 100).toFixed(0)}%.` };
  ...
  },
  {
    ...
    apply: (ctx) => {
      ...
      if (s.score > 0.25)  return { signal: "FAVOURED",  detail: `Net +${(s.score * 100).toFixed(0)} — note balance positive.` };
      if (s.score < -0.15) return { signal: "UNDERDOG", detail: `Net ${(s.score * 100).toFixed(0)} — note balance negative.` };
...
function combineSignals(results) {
  ...
  if (avg > 0.35)  return { signal: "FAVOURED",    detail: "Majority positive." };
  if (avg < -0.35) return { signal: "UNDERDOG",   detail: "Majority negative." };
...
function dollarFmt(x) {
  if (x == null || !isFinite(x)) return "—";
  return Math.round(x).toLocaleString() + " Elo";
}
...
const kpiDefs = () => {
  ...
  return [
    ...
    { id: "kpi-er",     label: "Expected rating change", value: k.expectedReturn,  fmt: (v) => pctFmt(v),    mod: k.expectedReturn >= 0 ? "positive" : "negative", valueMod: k.expectedReturn >= 0 ? "up" : "down", sub: `${state.horizon}-step · scenario` },
    { id: "kpi-lower",    label: "95% lower bound",         value: k.lowerBound,           fmt: (v) => pctFmt(v),    mod: "negative", valueMod: "down",           sub: "5th percentile" },
    { id: "kpi-stability", label: "Form consistency",          value: k.stability,          fmt: (v) => v.toFixed(2), mod: "accent",                              sub: "Normalised dispersion" },
...
function renderDetail(onComplete) {
  ...
  $("#detail-subtitle").textContent = `Game: ${stock.sector} · Elo ${priceFmt(stock.price)} (${pctFmt(stock.change)}) · Drift μ=${(stock.mu * 100).toFixed(1)}%, vol σ=${(stock.sigma * 100).toFixed(1)}%.`;
...
function renderChart(mc, stock, onComplete) {
  ...
  const s = mc.summary;
  const captionText = `${mc.nPaths.toLocaleString()} seeded scenario paths over ${mc.days} match steps. Expected rating change ${pctFmt(s.expectedReturn)}, median ${pctFmt(s.medianReturn)}, 95% CI ${pctFmt(s.ci95Low)} to ${pctFmt(s.ci95High)}, probability of gain ${(s.probUp * 100).toFixed(0)}%.`;
...
function renderSentiment(stock) {
  ...
  const posts = s.posts.length ? s.posts : [{ sent: "neu", author: "@scenario", handle: "now", text: "No recent scenario notes for this team." }];
...
function renderExtraPanel() {
  ...
  const cell = 56, gap = 2, headW = 70;
  const w = headW + n * (cell + gap) + 12;
  const h = headW + n * (cell + gap) + 12;
  const winProb = (a, b) => 1 / (1 + Math.pow(10, (b.price - a.price) / 400));
  let cells = "";
  for (let i = 0; i < n; i++) {
    for (let j = 0; j < n; j++) {
      if (i === j) {
        cells += `<rect x="${headW + j * (cell + gap)}" y="${headW + i * (cell + gap)}" width="${cell}" height="${cell}" fill="#0a0b0e"/>`;
        continue;
      }
      const p = winProb(teams[i], teams[j]);
      const alpha = Math.abs(p - 0.5) * 1.6;
      const color = p > 0.5 ? `rgba(255, 43, 214, ${alpha})` : `rgba(255, 51, 102, ${alpha})`;
      cells += `
        <rect x="${headW + j * (cell + gap)}" y="${headW + i * (cell + gap)}" width="${cell}" height="${cell}" fill="${color}" stroke="#191c23" stroke-width="0.5"><title>${teams[i].ticker} vs ${teams[j].ticker}: ${(p * 100).toFixed(0)}% win</title></rect>
        <text x="${headW + j * (cell + gap) + cell / 2}" y="${headW + i * (cell + gap) + cell / 2 + 4}" fill="#d6dce4" font-size="11" text-anchor="middle" font-family="JetBrains Mono, monospace">${(p * 100).toFixed(0)}%</text>
      `;
    }
  }
  const rowLabels = teams.map((t, i) =>
    `<text x="${headW - 6}" y="${headW + i * (cell + gap) + cell / 2 + 4}" text-anchor="end" fill="#d6dce4" font-size="10" font-family="JetBrains Mono, monospace">${t.ticker}</text>`
  ).join("");
  const colLabels = teams.map((t, j) =>
    `<text x="${headW + j * (cell + gap) + cell / 2}" y="${headW - 12}" text-anchor="middle" fill="#d6dce4" font-size="10" font-family="JetBrains Mono, monospace">${t.ticker}</text>`
  ).join("");
  container.innerHTML = `
    <div style="overflow-x:auto;">
      <svg viewBox="0 0 ${w} ${h}" width="${w}" height="${h}" role="img" aria-label="Head to head win probability matrix from Elo ratings for ${n} teams.">
        <text x="6" y="14" fill="#7f8693" font-size="9" font-family="JetBrains Mono, monospace">ROW vs COL</text>
  ...
  if (body) {
    ...
    for (let i = 0; i < n; i++) for (let j = i + 1; j < n; j++) {
      const p = winProb(teams[i], teams[j]);
      rows += `<tr><td>${teams[i].ticker} vs ${teams[j].ticker}</td><td>${(p * 100).toFixed(0)}%</td></tr>`;
    }
    body.innerHTML = rows;
  }
}
...
function renderExtra3Panel() {
  ...
  if (!container) return;
  const data = [{"name":"RD_A combined","units":"−30pts","down":-3.8,"up":4.2},{"name":"r_A − r_B","units":"+50pts","down":-2.6,"up":2.6},{"name":"τ system constant","units":"+50%","down":-0.7,"up":0.9},{"name":"Map-pool advantage","units":"+1 map","down":-1.2,"up":1.4},{"name":"Form Δ-Elo trend","units":"+30/90d","down":-0.8,"up":1.1},{"name":"Roster-stability","units":"−1 swap","down":-0.4,"up":0.6}];
  ...
  const bars = data.map((d, i) => {
    ...
    const upBar = `<rect x="${xZero}" y="${y}" width="${ux.toFixed(1)}" height="${bh}" fill="#ff2bd6" fill-opacity="0.72"/>`;
    ...
    const upText = `<text x="${xZero + ux + 4}" y="${y + bh / 2 + 4}" fill="#ff2bd6" font-size="10" font-family="JetBrains Mono, monospace">+${d.up.toFixed(1)}%</text>`;
...
function renderExtra4Panel() {
  ...
  if (!container) return;
  const X = {"name":"r_A−r_B","values":[-200,-100,-50,0,50,100,200]};
  const Y = {"name":"RD_*","values":[60,90,120,150,180]};
  const label = "P(A wins Bo5) %";
  const surface = (x, y) => 1 / (1 + Math.exp(-(x / 100 - y / 200)));
  const palette = ["#1a2029", "#b31e96", "#ff2bd6", "#ff7be5", "#ff3366"];
  const glyphs = [".", "·", "=", "▲", "■"];
